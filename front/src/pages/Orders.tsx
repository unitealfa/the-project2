...
const Orders: React.FC = () => {
  ...
  const canAccess = user && ['admin', 'confirmateur'].includes(user.role);
    ...
    <div style={{ padding: '1.5rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
        <div>
          ...
          <p style={{ margin: 0, color: '#666' }}>Synchronisées automatiquement depuis Google Sheets</p>
        </div>
        <button
          onClick={fetchOrders}
          disabled={loading}
